...
public class FirestoreRepository implements EventStoreRepository {
    ...
    public List<DomainEvent> getEventsBy(final AggregateRootId aggregateRootId) {
        ...
        return query.stream().map(document -> getDomainEvent(document)).collect(Collectors.toList());

    }

    private DomainEvent getDomainEvent(final QueryDocumentSnapshot document) {
